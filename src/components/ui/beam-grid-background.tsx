...
const BeamGridBackground: React.FC<BeamGridBackgroundProps> = ({
        ...
        const draw = () => {
            ...
            allBeams.forEach((beam) => {
                ...
                if (beam.dir === "x") {
                    ...
                    const start = -beamLength + (beam.offset % (canvas.width + beamLength));
                    ...
                    if (beam.offset > canvas.width + beamLength) beam.offset = -beamLength;
                } else {
                    const x = beam.x * gridSize;
                    const beamLength = gridSize * 1.5;
                    const start = -beamLength + (beam.offset % (canvas.height + beamLength));
                    ...
                    beam.offset += idle ? beam.speed * idleSpeed * 60 : beam.speed * 60;
                    if (beam.offset > canvas.height + beamLength) beam.offset = -beamLength;
                }
            });
    ...
    return (
        ...
        >
            ...
            {showFade && (
                <div
                    className="pointer-events-none absolute inset-0 bg-white dark:bg-[#080808]"
                    style={{
