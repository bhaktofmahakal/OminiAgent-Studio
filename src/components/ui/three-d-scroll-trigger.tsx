...
function ThreeDScrollTriggerRowImpl({
    ...
    useEffect(() => {
        ...
        if (block) {
            ...
            unitWidthRef.current = scrollWidth;
            const containerWidth = container.offsetWidth;
            const needed = Math.max(3, Math.ceil(containerWidth / scrollWidth) + 2);

            if (needed !== numCopies && isFinite(needed)) {
                // Use requestAnimationFrame to avoid synchronous state updates during layout
